...
function validateProposal(raw: unknown): HypothesisProposal {
  ...
  if (p.childIssues.length > 12)
    throw new Error("proposal.childIssues exceeds max 12 items");
...
function extractProposalBlocks(body: string): string[] {
  const out: string[] = [];
...
function extractProposalsFromBody(body: string): {
  proposals: HypothesisProposal[];
...
function buildThemeIssueBody(p: HypothesisProposal, discussionIssueIdentifier: string | null): string {
  const lines: string[] = [];
  lines.push(`# ${p.title}`);
  if (p.summary) {
    lines.push("");
...
function buildChildIssueBody(child: ChildIssueProposal, theme: HypothesisProposal): string {
  const lines: string[] = [];
...
/**
 * Ensure the pipeline always ends with a `[Publish]` step that runs
 * `paper-typeset`. PI is supposed to include this in every proposal, but
 * we add it defensively so the workflow is guaranteed even if the
 * hypothesis-proposal block omitted it.
 */
function ensurePublishStep(proposal: HypothesisProposal): HypothesisProposal {
  const hasPublish = proposal.childIssues.some(
    (c) => /^\s*\[publish\]/i.test(c.title),
  );
  ...
  const augmented: HypothesisProposal = {
    ...
    childIssues: [
      ...proposal.childIssues,
      {
        ...
        priority: "medium",
        deliverable:
          "paper/dist/main.{html,pdf,tex} produced via the paper-typeset skill",
        dependsOn,
