...
description: |
  Zero-context cross-model verification that every numeric claim and
  comparison in the paper matches raw result files. Use before submission
  to catch confirmation bias (rounding drift, best-seed cherry-picks,
  config mismatches, delta arithmetic errors, scope overclaims).
metadata:
  classification: research-core
  sources:
    - kind: github-file
      repo: wanshuiyin/Auto-claude-code-research-in-sleep
      path: skills/skills-codex/paper-claim-audit/SKILL.md
